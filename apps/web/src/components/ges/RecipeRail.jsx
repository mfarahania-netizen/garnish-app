 ...
 */
export default function RecipeRail({ title, icon: Icon, items = [], onSeeAll, onOpen, onSave, saved = {} }) {
  ...
  return (
    <Box component="section" style={{ marginBlockStart: 'var(--g-space-6)' }}>
      ...
      >
        {items.map((it) => (
          <Box key={it.recipeId} style={{ inlineSize: 188, flexShrink: 0 }}>
            <RecipeCard
              compact
              title={it.title}
              ...
              saved={!!saved[it.recipeId]}
              onSave={() => onSave?.(it.recipeId)}
