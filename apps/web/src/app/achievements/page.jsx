    ...
    return (
      <Box style={{ ...card, padding: 'var(--g-space-4)', display: 'flex', alignItems: 'center', gap: 'var(--g-space-3)' }}>
        ...
        <Box style={{ flex: 1, minInlineSize: 0 }}>
          <Text component="div" style={{ fontFamily: 'var(--g-font-fa)', fontSize: 'var(--g-font-size-16)', fontWeight: 800, color: 'var(--g-color-text-primary)' }}>هفتهٔ شلوغی بود</Text>
          <Text component="div" style={{ fontFamily: 'var(--g-font-fa)', fontSize: 'var(--g-font-size-12)', color: 'var(--g-color-text-secondary)', marginBlockStart: 3 }}>{streak.kindMessage || 'از نو شروع کنیم؟ اولین پختِ امروز، رشتهٔ تازه.'}</Text>
        </Box>
      </Box>
    );
  }
  ...
  return (
    <Box style={{ ...card, position: 'relative', overflow: 'hidden', padding: 'var(--g-space-4)' }}>
      ...
      <Text component="p" style={{ position: 'relative', display: 'flex', gap: 'var(--g-space-1)', alignItems: 'flex-start', margin: 'var(--g-space-3) 0 0', fontFamily: 'var(--g-font-fa)', fontSize: 'var(--g-font-size-12)', lineHeight: 'var(--g-leading-body)', color: 'var(--g-color-text-muted)' }}>
        <IconInfoCircle size={12} stroke={1.8} aria-hidden="true" style={{ flexShrink: 0, marginBlockStart: 2 }} />یک هفتهٔ مهلت داری — اگه یه هفته نپزی، رشته نمی‌شکنه.
      </Text>
    </Box>
  );
}
