...
describe('SmsService Melipayamak transport', () => {
  ...
  it.each(['-2', '14', 'not-a-provider-result'])(
    ...
    async (providerResult) => {
      (global.fetch as jest.Mock).mockResolvedValue(response(`<string>${providerResult}</string>`));

      const error = await new SmsService().sendOtpCode('09125859634', '123456').catch((caught) => caught);
  ...
  it('allows raw OTP logging only in an explicit development or test environment', async () => {
    ...
    process.env.NODE_ENV = 'test';
    await expect(new SmsService().sendOtpCode('09125859634', '123456')).resolves.toBeUndefined();
    expect(info).toHaveBeenCalledWith('[dev-sms] otp 0912***34 code=123456');

    info.mockClear();
    process.env.NODE_ENV = 'staging';
    await expect(new SmsService().sendOtpCode('09125859634', '654321'))
      .rejects.toThrow('sms_provider_not_configured');
  ...
  it('aborts a hung provider call at the bounded timeout', async () => {
    ...
    const assertion = expect(new SmsService().sendOtpCode('09125859634', '123456'))
      .rejects.toThrow('sms_send_failed');
