...
export class AgenticChatService {
  ...
  private systemPrompt(allergens: string[]): string {
    ...
    return [
      ...
      'کارها: می‌توانی برای کاربر کار هم انجام بدهی — «این رو ذخیره کن» → add_favorite؛ «موادشو بریز تو لیستِ خرید» → add_recipe_to_shopping_list؛ «فلان روز فلان وعده فلان غذا رو بذار» → add_to_meal_plan (روز + وعده + id). همه با همان id رسپی. این‌ها را فقط وقتی کاربر صریحاً خواست انجام بده، و بعد کوتاه تأیید کن.',
