...
- Recommendation intelligence (E18/E43):
  ...
  [E18/E43-A13 Limited Dev Shadow Experiment Execution](execution/E18_E43_A13_FOUNDER_APPROVED_LIMITED_DEV_SHADOW_EXPERIMENT_EXECUTION_REPORT.md) ·
  [E18/E43-A13 design doc](recommendation/E18_E43_A13_FOUNDER_APPROVED_LIMITED_DEV_SHADOW_EXPERIMENT_EXECUTION.md) — Founder-approved, dev-only, shadow-only experiment EXECUTION: bounded run (A11 lab + A9 dev-traffic + A8 trace analysis) → redacted run ledger + result analysis + rollback/kill-switch drills + execution dossier, behind a default-OFF, admin-guarded, allow-run-gated route; **no public endpoint**, no live ranking/response change, no trace exposed to users, promotion never granted, Founder review of results required (next gate A14) ·
  [E18/E43-A14 Founder Results Review + Safe Limited Activation Plan](execution/E18_E43_A14_FOUNDER_RESULTS_REVIEW_SAFE_LIMITED_ACTIVATION_PLAN_REPORT.md) ·
  [E18/E43-A14 design doc](recommendation/E18_E43_A14_FOUNDER_RESULTS_REVIEW_SAFE_LIMITED_ACTIVATION_PLAN.md) — reads A13 evidence → Founder results review + readiness matrix (production **red**) + 14 mandatory guardrails + safe limited activation plan (≤5 users / ≤240 req / ≤24h, dev_internal_shadow_only) + blocker classifier + dry-run + decision pack, behind a default-OFF, admin-guarded, dry-run-gated route; **plans/validates only — never activates**; no public endpoint, no live ranking/response change, no trace exposed to users, Founder decision required (next gate A15)
- AI Core (E47):
...
- Recommendation — A13 limited dev shadow experiment execution gate: [e18_e43_a13_limited_dev_shadow_experiment_execution_results.json](qa/recommendation/e18_e43_a13_limited_dev_shadow_experiment_execution_results.json)
- Recommendation — A14 founder results review + activation plan gate: [e18_e43_a14_founder_results_review_activation_plan_results.json](qa/recommendation/e18_e43_a14_founder_results_review_activation_plan_results.json)
