...
- **⚠️ BLOCKER for R16 closure:** a **pre-existing** unguarded destructive cron `governance/data-retention.service.ts` (`@Cron('0 0 1 * *')` → `userEvent.deleteMany` older than 1y) is **ACTIVE** (wired via AppModule) with no dry-run/env-guard/approval — contradicting the audit's "no retention cron" and the dry-run-first policy; also a dual-deletion path on `userEvent`. Out of E39-1E's allowed-files scope → flagged, not modified. **R16 stays OPEN** until this cron is guarded/deprecated and a controlled prune is approved.
