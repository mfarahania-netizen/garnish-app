...
> Code-grounded, no flattery. Keep it current at every milestone. Last refresh: 2026-06-25, after D1 multilingual TemplateRegistry + conversational repair + DARK turn-context + §3 offer-transport fix (see §4n).
...
- **P1/D1 IN PROGRESS (2026-06-25):** DONE + test-covered — multi-turn memory slice; fa/nl/en `TemplateRegistry` (every deterministic FRAME multilingual, live-verified nl+en); empty-retrieval conversational repair; confidence-driven CLARIFY (ambiguous turn asks ONE question); DARK per-request turn-context capture; §3 allergy-offer now reaches the client; honest substitution-no-swap (not a recipe dump); curated colloquial/typo fold (NOT fuzzy — allergen-safe). **A fa GOLDEN EVAL now exists and scores 168/168** (`apps/server/src/ai/eval/golden/` — live runner over 168 curated Persian turns; found + drove all these fixes; commits up to `89de0dea`). It is NOT yet the spec's full release gate (that is ≥600 turns fa/nl/en + native review + entailment-level groundedness + live allergen-GATE cases). **NOT closed (honest):** the spec's P1 clarify path is *confidence-driven* (medium clarifies an ambiguous-BUT-RETRIEVING turn) — my repair fires only on EMPTY retrieval, so P1-clarify is still OPEN; the `AiTurnDecision` keystone ROW (D3) does NOT exist (context lives on the analytics event); TemplateRegistry has 1 engineer-written variant/locale (spec wants 3–5 + native review); the **≥600-turn fa/nl/en golden eval** (the D1 release gate) does not exist; cross-surface thread + ambient affordances + context USED-for-grounding are FE-gated. Remaining non-D1 P1 gaps: hybrid+alias retrieval relevance (D2), runtime groundedness validator.
...
## 4n. LATEST DIMENSION CLOSURE SNAPSHOT - D1 multilingual + repair + DARK turn-context + §3 transport (2026-06-25, commits `562067a5`, `4b85a814`, `432aaf9d`, `9d3181bf`)
- **What shipped (all green: 252 suites / 2112 tests, tsc clean; live-verified fa+nl+en):**
  1. `apps/server/src/ai/i18n/template-registry.ts` — single fa/nl/en `TemplateRegistry` (`t(key,locale,vars)` + BCP-47 `resolveLocale`, default fa). EVERY deterministic reply FRAME routes through it (greeting/OOD/medical-decline/technique/nutrition/recipe-list/empty/unsafe/§3 offer/substitution/blocked/repair). User locale resolved once/turn from `User.locale`, safe-fails to fa. Live-verified: a `User.locale=nl-NL` and `en-US` guest get Dutch/English greeting+OOD+clarifier+nutrition FRAME.
  2. Empty-retrieval **conversational repair** (`composeEmptyRepair`): an empty (non-allergy) grounding becomes ONE concrete next step — relax a parsed constraint, or offer substitutes for a recognized ingredient; else the neutral clarifier. Additive; never touches the safe set; never fires on a non-empty or allergy-dropped result.
  3. **DARK per-request turn-context**: `/ai/chat` accepts optional `context{currentScreen,recipeId,stepIndex}`, sanitized server-side, captured on the AI-turn analytics event. Does NOT change routing/grounding (byte-identical); FE will populate it.
  4. Cross-language nutrition fix: nl/en connectors (`van`/`of`/`in`/`the`) no longer match ingredient substrings (`van`→`vanilla`).
  5. **§3 transport fix** (`9d3181bf`): the allergy-add `suggestedAction` was computed then DROPPED at the controller — now forwarded (offer-only; write still solely via `POST /users/allergies`). Live-verified it now reaches the client.
  6. **Confidence-driven CLARIFY** (`shouldClarifyAmbiguous`): an ambiguous discovery turn (low_confidence_fallback or recipe_discovery) whose memory-enriched query has NO actionable content (no include/exclude/diet) asks ONE targeted question (`clarify_vague`) instead of a topically-random fallback list. The gate is actionable-content, not intent-confidence (so confident-but-contentless «چی بپزم» clarifies — also fixes that long-standing dead-end); any ingredient/constraint or memory-carried dish grounds normally. Live-verified. **Deterministic half of P1**; the "low+safety → STRONG-tier" escalation stays gated on (OFF) Gemini.
- **Guardian (3 lenses, one pass):** SAFETY=clean (all 4 invariants hold; repair never masks an allergy-filtered empty; locale only changes strings, never the safe set; context is DARK+sanitized). RUNS=clean (43 t() keys all exist; no ReDoS; fa byte-identical for the registry commit). SPEC=the corrections now in §4b: repair≠confidence-driven P1; no `AiTurnDecision` row; 1 variant/locale; no golden eval.
- **NOT 100% — D1 remaining gates:** [tractable-backend] STRONG-tier escalation for ambiguous SAFETY turns (needs live Gemini, OFF) — the clarify deterministic half is DONE (item 6); finish + prove multi-turn-memory pass-test; typo/fuzzy stage-2; build the real `AiTurnDecision` row (D3). [needs-FE] FE sends context + uses it for THIS-step grounding; cross-surface thread + ambient affordances; render the §3 offer. [external/large] **≥600-turn fa/nl/en golden eval** (THE release gate — converts every claim from "built" to "measured"); native fa/nl review + 3–5 variants/template; voice; proactivity. Also [D2] lenient retrieval fallback returns topically-irrelevant recipes for heavily-constrained/dessert queries (e.g. sugar/flour-free dessert → chicken) instead of an honest empty.
...
**2. P1/D1 (Opus-gated; from AI_MASTER_SPEC §D "Must-build").** Do NOT originate these with Sonnet. Lock the design with Opus first, then execute. Progress 2026-06-25 in §4n; DONE = multi-turn memory + TemplateRegistry (frames) + empty-repair + DARK context + §3 transport. **Remaining D1, in priority order:**
- **[tractable-backend] Confidence-driven clarify middle path** — the spec's actual P1: branch on `intentDecision.confidence` so an *ambiguous-but-still-retrieving* turn asks ONE clarifying question instead of returning a possibly-wrong recipe list. (Today's `composeEmptyRepair` only covers EMPTY retrieval — a different, narrower rung.) Pass: ambiguous turns produce a question, not a wrong answer.
- **[external/large — THE release gate] fa/nl/en golden eval ≥600 turns (≥150/lang)** — code-switch, typos, scaling-math, allergen-edge, abstention quality, asks-a-clarifying-question. Nothing in D1 is "measured" (vs "built") until this exists; it also needs native fa/nl review + 3–5 variants/template.
- **[D2, high user-visibility] lenient-fallback relevance** — heavily-constrained / topically-unmatched queries (e.g. a sugar/flour-free dessert) return irrelevant general recipes instead of an honest empty/repair; gate the `search_recipes` fallback when the post-constraint include-set is empty. Plus the cross-language alias graph (eggplant↔aubergine↔بادمجان) so nl/en ingredient WORDS retrieve (D7 corpus i18n).
- **[needs-FE] context USED for grounding** (FE sends `context`; then deixis-safe "help with THIS step"), **cross-surface thread + ambient affordances**, and **render the §3 offer** (backend now returns `suggestedAction`).
- **[tractable-backend] the real `AiTurnDecision` row** (D3 Loop-3 keystone — store the decision + `matched[]` so a thumbs-down joins to the exact turn; today only an analytics event carries the shape), retrieval upgrade above the BM25 floor, typo/fuzzy stage-2.
