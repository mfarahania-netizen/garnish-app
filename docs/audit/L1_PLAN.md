...
4. **PriorService** (empirical-Bayes shrinkage `(κμ+n·x̄)/(κ+n)`) + جدولِ RecipePrior، seed از دادهٔ ادیتوریالِ curated (نقشهٔ دروازهٔ اروپا + تقویمِ مناسبت)؛ n=0 ⇒ دقیقاً priorِ curated. wire به‌عنوان WeightSourceِ کوهورت‌آگاه + تعمیمِ coldStartWeightBlend به person→cohort→population.
