...
5. ✅ **DONE (default-OFF، byte-identical)** — **degradationِ جمعی + محافظتِ اقلیت** — **در رنکرِ زنده** (نه یک‌خطیِ سایه): جملهٔ افزایشیِ کراندار در اسلاتِ post-linear، seed از محبوبیتِ curated؛ محافظتِ اقلیت = ۳ لایه (کرانِ نامتقارن + گیتِ override با سیگنالِ شخصی + دانه‌بندیِ کوهورت)؛ **invariant: سیگنالِ شخصیِ مثبت ⇒ امتیاز هرگز کم نشود** (property-test). **ساخته‌شده:** `recipePriorSlateTerm` پیش‌فرض **LIFT-ONLY** (penMult=0) → invariant **بی‌قید** برقرار؛ هاردفلور برای حالتِ دوطرفهٔ آینده؛ فعال‌سازی با `L1_PRIOR_STEP5_WEIGHT>0` (گیتِ offline-replay). جزئیات: `L1_STEP5_MINORITY_SPEC.md`.
