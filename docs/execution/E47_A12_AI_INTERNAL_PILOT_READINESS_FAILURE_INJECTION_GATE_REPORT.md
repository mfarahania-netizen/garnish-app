...
- **guard_order (8):** missing snapshot fail-fast; injection / inbound-safety / per-request-cost block **before** provider (0 calls); **cost-before-safety precedence** pinned; persisted daily budget blocks before provider (live-configured); **budget DB error → fail-closed**; outbound nutrition guard blocks unsafe nutrition **after** provider; AICallLog persistence failure never breaks the call.
