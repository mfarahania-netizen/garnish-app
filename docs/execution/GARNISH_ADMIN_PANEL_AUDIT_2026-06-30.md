...
- **P1-7/8 + P2-1 (بازچینیِ nav):** ✅ از نوعِ-متریک به **job-based** (۶ گروه؛ realtime→مرکزِ فرماندهی، safety جدا طبق §۷)؛ فقط آرایهٔ GROUPS عوض شد — تب‌ها/routing دست‌نخورده. **زنده:** هر ۱۲ تب حاضر، کلیکِ Safety→`?tab=safety` بی‌crash؛ P2-1: railِ آیکن‌اونلیِ narrow تأیید. پوش (`9dbd071e`).
- **P1-16 (any) + حادثه:** ✅ `where/orderBy/data: any`→typeهای Prisma. **اما:** ادیتِ L149 (استفادهٔ `data.email` در WHERE) خطای TS داد که تأییدِ ناقصِ من (tail-grep) از دست داد → buildِ master ~۱۰دق شکست + recompileها بلاک شد → P1-6 routes ۴۰۴ → کشف+رفع (narrowِ email). پوش (`b7898be4`). **درس:** کلِ لاگ را برای «Found N errors» چک کن، نه tailِ محدود.
- **P1-6 (observability cabin):** ✅ ۶ تبِ کاربر (Timeline/Signals/Profile/Consent/AI/Tickets) + ۳ endpointِ جدید (consent/ai-calls/tickets، PII-conscious؛ allergy مخفی، AI فقط metadata). **زنده:** هر ۶ endpoint ۲۰۰ + شکلِ داده دقیقاً با read‌های کابین می‌خورد (۵۰ev/۶sig/۴dim/۸consent/۳۰ai). پوش (`8252bb24`).
- **✅✅✅ کلِ ممیزی (P0..P2) تمام + پوش.** دو UIِ drawer (P1-4 triage + P1-6 cabin) با endpoint+build+شکلِ‌داده تأیید شد، نه بازکردنِ بصری — محدودیتِ تعاملِ previewِ headless با `<tr onClick>` (کلیکِ دکمه/تب کار می‌کند؛ مالک live می‌بیند).
- **P2-3 (خطا):** ✅ بخشِ [قطعی] («export failure→toast») با حذفِ export در P0-4 moot؛ UIِ کاربر/تیکت خطا را با ErrorState/errLine نشان می‌دهد؛ counterِ parsing [احتمالاً]/قابل‌تحمل (سند) — رها شد.
