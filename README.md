...
- **AI — REBUILD IN PROGRESS (founder priority).** The old aspirational AI design specs (the big "dimensions"
  master spec + fragments) were RETIRED — they designed a lot but didn't ship a working assistant. Live Gemini
  chat is now **ON** (grounded in the corpus, behind a fail-closed allergy + pork/observance gate); recent work
  made it answer BY CRITERIA (region/mealType/cookingTime), deliver real recipes inline, know the user
  (UserBehaviorProfile + UserFact memory), and fixed cost/token logging. The live plan is to rebuild it into a
  true **agentic companion** (Claude-class brain + tool-use + actions + nutrition math + admin-analytics +
  proactive notifications + cited web knowledge), measured by an **eval harness** — source of truth:
  [`docs/audit/AI_COMPANION_REARCHITECTURE.md`](docs/audit/AI_COMPANION_REARCHITECTURE.md) +
  [`AI_WORLDCLASS_CAPABILITY_MAP.md`](docs/audit/AI_WORLDCLASS_CAPABILITY_MAP.md) +
  [`CHAT_INTELLIGENCE_EXECUTION_PLAN.md`](docs/audit/CHAT_INTELLIGENCE_EXECUTION_PLAN.md). Honest gap: the
  user-data CAPTURE pipelines (onboarding→profile, Cook Mode→cook_complete, "remember this"→UserFact) are mostly
  unwired, so the data is still sparse.
...
- **Live Gemini chat is ON in local dev** (founder-authorized 2026-06-25, VPN on): `AI_PROVIDER=gemini` + `AI_LIVE_ENABLED=true` + `AI_CHAT_LIVE_ENABLED=true` + a real key in the gitignored `apps/server/.env`. Replies are natural grounded Gemini text; the SAFETY gate (allergy + pork/observance) holds live, fail-closed, verified end-to-end. The **EU/production** flip stays legal-gated. Still no model-driven tool-use loop, no vision, no streaming, no medical advice — those are the agentic rebuild (see the AI snapshot above). The brain is planned to move to a **Claude-class** model.
